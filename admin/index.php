...
function showPages()
{
    ...
    for ($i = 0; $i < $xoopsDB->getRowsNum($result); $i++) {
        [$page, $title] = $xoopsDB->fetchRow($result);

        [$page, $revs] = $xoopsDB->fetchRow($result2);

        echo '<tr class="' . (($i % 2) ? 'even' : 'odd') . '"><td><a href="index.php?page=' . $page . '&op=history">' . $page . '</a></td>
        <td>' . htmlspecialchars($title, ENT_QUOTES | ENT_HTML5) . '</td>
        <td>' . $revs . '</td>
...
function showHistory($page)
{
    ...
    for ($i = 0; $i < $xoopsDB->getRowsNum($result); $i++) {
        [$id, $title, $body, $lastmodified, $uid] = $xoopsDB->fetchRow($result);

        echo '<tr class="' . (($i % 2) ? 'even' : 'odd') . '"><td><a href="index.php?page=' . $page . '&op=display&id=' . $id . '">' . htmlspecialchars($title, ENT_QUOTES | ENT_HTML5) . '</a></td>';
        ...
        echo '<td><a href="index.php?page='
             ...
             . '">'
             . _AM_WIKIMOD_VIEW
             . '</a> | <a href="javascript:restoreRevision(\''
             . $id
             . '\');">'
             . _AM_WIKIMOD_RESTORE
...
function showPage($page, $id)
{
    ...
    [$title, $body, $lastmodified, $uid] = getRevision($page, $id);

    echo '<p style="padding-bottom: 2px; border-bottom: 1px solid #000000;">'
         . _MD_WIKIMOD_PAGE
         . ": <strong>$page</strong> - "
         . _MD_WIKIMOD_LASTMODIFIED
         . ' <i>'
         . date($xoopsModuleConfig['date_format'], @strtotime($lastmodified))
         . '</i> '
         . _MD_WIKIMOD_BY
         . ' <i>'
         . getUserName($uid)
    ...
    echo '<h1>' . htmlspecialchars($title, ENT_QUOTES | ENT_HTML5) . '</h1>';

    echo wikiDisplay($body);

    echo '<p style="text-align: right; padding-top: 2px; border-top: 1px solid #000000;"><a href="index.php?page='
         . $page
         . '&op=history">--&gt; '
         . _BACK
         . '</a> | <a href="javascript:restoreRevision(\''
...
switch ($op) {
    ...
    case 'display':
        ...
        break;
    case 'restore':
        [$title, $body] = getRevision($page, $id);
